#include "../header/lve_renderer.hpp"
...
namespace lve {
	...
	{
		assert(isFrameStarted && "Can't call beginSwap if frame is not in progress");
		assert(commandBuffer == getCurrentCommandBuffer() && "Can't begin renderPass");
	...
	{
		assert(isFrameStarted && "Can't call endSwap if frame is not in progress");
